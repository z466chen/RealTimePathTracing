...
void read_trb(out Reservior trb, int dx, int dy) {
    vec4 data1 = texture(trb_tex1, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
    vec4 data2 = texture(trb_tex2, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
    vec4 data3 = texture(trb_tex3, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
    vec4 data4 = texture(trb_tex4, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
    vec4 data5 = texture(trb_tex5, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
    vec4 data6 = texture(trb_tex6, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
    vec4 data7 = texture(trb_tex7, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
...
void read_srb(out Reservior srb, int dx, int dy) {
    vec4 data1 = texture(srb_tex1, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
    vec4 data2 = texture(srb_tex2, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
    vec4 data3 = texture(srb_tex3, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
    vec4 data4 = texture(srb_tex4, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
    vec4 data5 = texture(srb_tex5, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
    vec4 data6 = texture(srb_tex6, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
    vec4 data7 = texture(srb_tex7, vec2(texCoord.x+(dx/window_size.x), 1 - texCoord.y-(dy/window_size.y)));
...
void pack_output(in Reservior srb) {
    outdata1 = vec4(srb.z.xv.rgb, 1.0f);
    outdata2 = vec4(srb.z.nv.rgb, 1.0f);
    outdata3 = vec4(srb.z.xs.rgb, 1.0f);
    outdata4 = vec4(srb.z.ns.rgb, 1.0f);
    outdata5 = vec4(srb.z.lo.rgb, 1.0f);
    outdata6 = vec4(srb.z.oid,srb.z.mid,srb.w,1.0f);
    outdata7 = vec4(srb.M,srb.W,0,1.0f);
}

float rand_seed_y = 2.0f;
...
bool test_geometry_similarity(vec3 n1, vec3 d1, vec3 n2, vec3 d2) {
    return (dot(n1, n2) > cos(radians(25.0f))) && (length(d1 - d2) < 0.05); 
}
...
vec3 get_p_hat(int oid, in Ray primaryRay, in vec3 xv, in vec3 nv, in vec3 xs, in vec3 ns) {
    Object obj = getObject(oid);
    MaterialInfo m = getMaterialInfo(obj, xv);
    vec3 N = normalize(nv);
    vec3 wo = normalize(-primaryRay.rd);
    vec3 dl_vec = xs - xv;
    ...
    if (m.m_type == 5) {
        // direct lighting 
        float pdf = 1.0f/(obj.obj_data_3.x);
        vec3 NN = normalize(ns);
        return (evalMaterial(m,ws, wo, N) * dot(NN, -ws) * dot(N, ws))/ (dl_distance*dl_distance*pdf);
    } else {
        return (evalMaterial(m,ws, wo, N) * 
        dot(N, ws))/pdfMaterial(m,ws, wo, N);
    }
}
...
void main() {
    ...
    int valid_samples[KERNEL_SIZE*KERNEL_SIZE];
    int middle = int((KERNEL_SIZE*KERNEL_SIZE-1)*0.5);
    int nos = 1;
    // valid_samples[0] = middle;
    ...
    for (int i = 0;i < MAX_INTERATIONS; ++i) {
        ...
        vec3 dir =  normalize(vtrans(transpose(viewMatrix), 
        vec3(((gl_FragCoord.x+dx)*2 - window_size.x)*h_ratio,
             ((gl_FragCoord.y+dy)*2 - window_size.y)*w_ratio,
             -1.0f)));
        Ray primaryRay = Ray(eyePos, dir);

        
        if (trb_qn.z.oid < 0 || !test_geometry_similarity(trb.z.nv,
            (trb.z.xv - eyePos),trb_qn.z.nv,(trb_qn.z.xv - eyePos))) {
            continue;
        }
        // calculate Jacobian determinant
        vec3 rsqn = (trb.z.xv - trb_qn.z.xs);
        vec3 nsqn = normalize(trb_qn.z.ns);
        vec3 qsqn = (trb_qn.z.xv - trb_qn.z.xs);
        // float jacob = abs(dot(nsqn,rsqn)/dot(nsqn,qsqn)) * pow(length(rsqn)/length(qsqn), 3.0f);
        float w = length(trb_qn.z.lo*(get_p_hat(trb.z.mid, 
            primaryRay,trb.z.xv, trb.z.nv, trb_qn.z.xs, trb_qn.z.ns)/
            get_p_hat(trb_qn.z.mid, primaryRay,trb_qn.z.xv, trb.z.nv, trb_qn.z.xs, trb_qn.z.ns)));
        
        vec3 delta;
        if (dot(trb.z.nv, -rsqn) > 0) {
    ...
    srb.M = min(srb.M, MAX_SPATIAL_DEPTH);

    srb.W = srb.w/(nos*length(srb.z.lo));
